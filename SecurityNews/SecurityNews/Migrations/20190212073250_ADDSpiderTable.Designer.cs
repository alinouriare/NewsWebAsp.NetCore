// <auto-generated />
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage;
using Microsoft.EntityFrameworkCore.Storage.Internal;
using SecurityNews.Models;
using System;

namespace SecurityNews.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20190212073250_ADDSpiderTable")]
    partial class ADDSpiderTable
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "2.0.3-rtm-10026")
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("ClaimType");

                    b.Property<string>("ClaimValue");

                    b.Property<string>("RoleId")
                        .IsRequired();

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("ClaimType");

                    b.Property<string>("ClaimValue");

                    b.Property<string>("UserId")
                        .IsRequired();

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.Property<string>("LoginProvider");

                    b.Property<string>("ProviderKey");

                    b.Property<string>("ProviderDisplayName");

                    b.Property<string>("UserId")
                        .IsRequired();

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.Property<string>("UserId");

                    b.Property<string>("RoleId");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.Property<string>("UserId");

                    b.Property<string>("LoginProvider");

                    b.Property<string>("Name");

                    b.Property<string>("Value");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens");
                });

            modelBuilder.Entity("SecurityNews.Models.Advertise", b =>
                {
                    b.Property<int>("AdId")
                        .ValueGeneratedOnAdd();

                    b.Property<byte>("Advlocation");

                    b.Property<string>("FromDate")
                        .IsRequired();

                    b.Property<string>("Link")
                        .IsRequired();

                    b.Property<string>("ToDate")
                        .IsRequired();

                    b.Property<byte>("flag");

                    b.Property<string>("gifPath")
                        .IsRequired();

                    b.HasKey("AdId");

                    b.ToTable("Advertis_Tbl");
                });

            modelBuilder.Entity("SecurityNews.Models.ApplicationRoles", b =>
                {
                    b.Property<string>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken();

                    b.Property<string>("Description");

                    b.Property<string>("Name")
                        .HasMaxLength(256);

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256);

                    b.Property<string>("RoleLevel");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasName("RoleNameIndex")
                        .HasFilter("[NormalizedName] IS NOT NULL");

                    b.ToTable("AspNetRoles");
                });

            modelBuilder.Entity("SecurityNews.Models.ApplicationUsers", b =>
                {
                    b.Property<string>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<int>("AccessFailedCount");

                    b.Property<string>("BirthDayDate");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken();

                    b.Property<string>("Email")
                        .HasMaxLength(256);

                    b.Property<bool>("EmailConfirmed");

                    b.Property<string>("Family");

                    b.Property<bool>("LockoutEnabled");

                    b.Property<DateTimeOffset?>("LockoutEnd");

                    b.Property<string>("Name");

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256);

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256);

                    b.Property<string>("PasswordHash");

                    b.Property<string>("PhoneNumber");

                    b.Property<bool>("PhoneNumberConfirmed");

                    b.Property<string>("SecurityStamp");

                    b.Property<bool>("TwoFactorEnabled");

                    b.Property<string>("UserImagePath");

                    b.Property<string>("UserName")
                        .HasMaxLength(256);

                    b.Property<byte>("gender");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedEmail")
                        .HasName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasName("UserNameIndex")
                        .HasFilter("[NormalizedUserName] IS NOT NULL");

                    b.ToTable("AspNetUsers");
                });

            modelBuilder.Entity("SecurityNews.Models.CategoryImpact", b =>
                {
                    b.Property<int>("CategoryImpactId")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(150);

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(150);

                    b.HasKey("CategoryImpactId");

                    b.ToTable("CategoryImpact_Tbl");
                });

            modelBuilder.Entity("SecurityNews.Models.CategoryPlatform", b =>
                {
                    b.Property<int>("CategoryPlatformtId")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(150);

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(150);

                    b.HasKey("CategoryPlatformtId");

                    b.ToTable("CategoryPlatform_Tbl");
                });

            modelBuilder.Entity("SecurityNews.Models.Comment", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<int>("DisLikeCount");

                    b.Property<string>("Email")
                        .IsRequired();

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(60);

                    b.Property<string>("IP");

                    b.Property<int>("LikeCount");

                    b.Property<string>("Message")
                        .IsRequired()
                        .HasMaxLength(2000);

                    b.Property<int>("NewsId");

                    b.Property<int>("ReplyID");

                    b.Property<string>("commentDate");

                    b.Property<string>("commentTime");

                    b.Property<bool>("status");

                    b.HasKey("Id");

                    b.HasIndex("NewsId");

                    b.ToTable("Comments_Tbl");
                });

            modelBuilder.Entity("SecurityNews.Models.News", b =>
                {
                    b.Property<int>("NewsId")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("Abstract")
                        .IsRequired();

                    b.Property<string>("CVE");

                    b.Property<int>("CategoryImpactID");

                    b.Property<int>("CategoryPlatformID");

                    b.Property<string>("Content")
                        .IsRequired();

                    b.Property<string>("IndexImage");

                    b.Property<string>("MetaDescription");

                    b.Property<string>("MetaTag");

                    b.Property<string>("NewsDate");

                    b.Property<DateTime>("NewsDateEN");

                    b.Property<string>("NewsTime");

                    b.Property<byte>("NewsType");

                    b.Property<string>("Path");

                    b.Property<byte>("PlaceNewsID");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(250);

                    b.Property<string>("UserID");

                    b.Property<int>("VisitCount");

                    b.HasKey("NewsId");

                    b.HasIndex("CategoryImpactID");

                    b.HasIndex("CategoryPlatformID");

                    b.HasIndex("UserID");

                    b.ToTable("News_Tbl");
                });

            modelBuilder.Entity("SecurityNews.Models.Poll", b =>
                {
                    b.Property<int>("PollId")
                        .ValueGeneratedOnAdd();

                    b.Property<bool>("Active");

                    b.Property<string>("PollEndDate");

                    b.Property<string>("PollStartDate");

                    b.Property<string>("Question");

                    b.HasKey("PollId");

                    b.ToTable("Poll_Tbl");
                });

            modelBuilder.Entity("SecurityNews.Models.PollOption", b =>
                {
                    b.Property<int>("PolloptionID")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("Answer");

                    b.Property<int>("PollID");

                    b.Property<int>("VouteCount");

                    b.HasKey("PolloptionID");

                    b.HasIndex("PollID");

                    b.ToTable("Polloption_Tbl");
                });

            modelBuilder.Entity("SecurityNews.Models.SiteSetting", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("MetaDescription");

                    b.Property<string>("MetaTag");

                    b.Property<string>("SiteTitle")
                        .IsRequired();

                    b.HasKey("id");

                    b.ToTable("SiteSetting_Tbl");
                });

            modelBuilder.Entity("SecurityNews.Models.SpiderModel.InternalLink", b =>
                {
                    b.Property<int>("InternalLinkId")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("IntrnalUrl");

                    b.Property<int>("MainlinkID");

                    b.Property<DateTime>("SpiderDate");

                    b.Property<bool>("SpiderStatus");

                    b.HasKey("InternalLinkId");

                    b.HasIndex("MainlinkID");

                    b.ToTable("InternalLink_tbl");
                });

            modelBuilder.Entity("SecurityNews.Models.SpiderModel.MainLink", b =>
                {
                    b.Property<int>("MainLinkId")
                        .ValueGeneratedOnAdd();

                    b.Property<DateTime>("DateNews");

                    b.Property<bool>("Status");

                    b.Property<string>("Titel");

                    b.Property<string>("Url");

                    b.Property<string>("XpathContent");

                    b.Property<string>("XpathTitle");

                    b.HasKey("MainLinkId");

                    b.ToTable("MainLink_tbl");
                });

            modelBuilder.Entity("SecurityNews.Models.SpiderModel.NewsSpider", b =>
                {
                    b.Property<int>("IdNews")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("Content");

                    b.Property<int>("IntrnalLinkID");

                    b.Property<DateTime>("NewsDate");

                    b.Property<bool>("Status");

                    b.Property<string>("Titel");

                    b.HasKey("IdNews");

                    b.HasIndex("IntrnalLinkID");

                    b.ToTable("NewsSpider_tbl");
                });

            modelBuilder.Entity("SecurityNews.Models.VisitorHits", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("DateTime");

                    b.Property<string>("IpAddress");

                    b.Property<int>("VisitHit");

                    b.HasKey("id");

                    b.ToTable("Visit_Tbl");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.HasOne("SecurityNews.Models.ApplicationRoles")
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.HasOne("SecurityNews.Models.ApplicationUsers")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.HasOne("SecurityNews.Models.ApplicationUsers")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.HasOne("SecurityNews.Models.ApplicationRoles")
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("SecurityNews.Models.ApplicationUsers")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.HasOne("SecurityNews.Models.ApplicationUsers")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("SecurityNews.Models.Comment", b =>
                {
                    b.HasOne("SecurityNews.Models.News", "TblNews")
                        .WithMany()
                        .HasForeignKey("NewsId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("SecurityNews.Models.News", b =>
                {
                    b.HasOne("SecurityNews.Models.CategoryImpact", "tblCategoryImpact")
                        .WithMany()
                        .HasForeignKey("CategoryImpactID")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("SecurityNews.Models.CategoryPlatform", "tblCategoryPlatform")
                        .WithMany()
                        .HasForeignKey("CategoryPlatformID")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("SecurityNews.Models.ApplicationUsers", "Users")
                        .WithMany()
                        .HasForeignKey("UserID");
                });

            modelBuilder.Entity("SecurityNews.Models.PollOption", b =>
                {
                    b.HasOne("SecurityNews.Models.Poll", "poll")
                        .WithMany("pollOption")
                        .HasForeignKey("PollID")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("SecurityNews.Models.SpiderModel.InternalLink", b =>
                {
                    b.HasOne("SecurityNews.Models.SpiderModel.MainLink", "tblMainLink")
                        .WithMany()
                        .HasForeignKey("MainlinkID")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("SecurityNews.Models.SpiderModel.NewsSpider", b =>
                {
                    b.HasOne("SecurityNews.Models.SpiderModel.InternalLink", "tblInternalLink")
                        .WithMany()
                        .HasForeignKey("IntrnalLinkID")
                        .OnDelete(DeleteBehavior.Cascade);
                });
#pragma warning restore 612, 618
        }
    }
}
